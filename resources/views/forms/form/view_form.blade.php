                ...
                <div class="panel-body">
                    <form id="user-form" action="{{ ($view_type === 'form') ? route('forms.responses.store', $form->code) : "#" }}" method="{{ ($view_type === 'form') ? "post" : "get" }}" autocomplete="off">
                        ...
                        <div class="text-left mt-20">
                            <button id="submit" type="{{ ($view_type === 'form') ? 'submit' : 'button' }}" class="btn btn-primary" data-loading-text="Please Wait..." data-complete-text="Submit Form">{{ trans('Soumettre_le_formulaire') }}</button>
                        </div>
                    </form>
                </div>
            @endif
