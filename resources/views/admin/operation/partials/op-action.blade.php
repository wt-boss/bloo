...
<ul class="dropdown-menu dropdown-menu-right" style="padding: 10px;">
    ...
    @if (auth()->user()->hasRole('Superadmin|Account Manager'))
        <li><a href="{{ route('startoperation', [$operation->id]) }}" class="btn btn-bloo-action mb-5" @php if(($operation->status === "EN COUR")||($operation->status === "TERMINER")){  echo "disabled='true'"  ; } @endphp>{{ trans('commencer_op') }}</a></li>
